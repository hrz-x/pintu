...
def extract_connected_components(
    input_path, output_folder, erosion_radius=5, dilation_radius=None
):
    """
    ...
    for region in regions:
        ...
        for y in range(min_row, max_row):
            for x in range(min_col, max_col):
                if (
                    labeled[y, x] == region.label and binary[y, x]  # type: ignore
                ):  # 只保留原始图像中的黑色像素 # type: ignore
        ...
        if bbox:
            ...
            center_offset = [
                round(center_x - original_center[0], 2),
            ...
            component_info = {
                ...
                "area": region.area,
                "size": {"width": cropped_img.width, "height": cropped_img.height},
                "bbox": {
                    "x": min_col,
                    "y": min_row,
...
if __name__ == "__main__":
    ...
    json.dump(
        candidates,
        open("./data/all.json", "w", encoding="utf-8"),
        ensure_ascii=False,
        indent=4,
    )
