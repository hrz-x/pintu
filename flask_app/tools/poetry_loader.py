...
class PlainDataLoader:
    def __init__(self, config_path: str = DATAS_CONFIG) -> None:
        ...
        with open(config_path, "r", encoding="utf-8") as config:
            ...
            self.id_table = {v["id"]: k for (k, v) in self.datasets.items()}
    ...
    def body_extractor(self, target: str) -> list:
        ...
        for filename in subpaths:
            if "excludes" in configs and filename in configs["excludes"]:
                continue
            with open(
                os.path.join(full_path, filename), mode="r", encoding="utf-8"
            ) as file:
                data = json.load(file)
                for poem in data:
                    body.append(poem)
    ...
    def extract_with_ids(self, ids: list) -> list:
        results = []
        for id in ids:
            results += self.body_extractor(self.id_table[id])
        return results
...
if __name__ == "__main__":
    ...
    all_poetrys = [
        poem for poem in all_poetrys if max([len(e) for e in poem["content"]]) <= 20
    ]
