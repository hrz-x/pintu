        ...
        for y in range(bbox[1], bbox[3]):
            for x in range(bbox[0], bbox[2]):
                if outline_alpha.getpixel((x, y)) > 0:
                    ...
                    if pos < dash_length:
                        outline.putpixel(
                            (x, y), (*ImageColor.getrgb(outline_color), 255)
                        )
...
if __name__ == "__main__":
    ...
    for font, font_file in fonts.items():
        ...
        for zi in list(hanzi.keys()):
            generate_hanzi_image(
                zi,
                font_file,
                f"data/hanzi_pic/{font}/{zi}.jpg",
                size=720,
                text_color="black",
                transparent=False,
            )
            ...
            generate_outline(
                f"data/hanzi_pic/{font}/{zi}.png",
                f"data/hanzi_pic/{font}/{zi}_outline.jpg",
                outline_color="black",
                outline_width=1,
                dash_length=20,
                gap_length=0,
